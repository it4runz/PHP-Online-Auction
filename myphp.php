...
$id=$_SESSION['adminid'];
	$query="update admin set image='$name' where id='$id'";
...
$tmp_name=$_FILES["image"]["tmp_name"];
	...
	echo $query="INSERT INTO `books`( `isbn`, `Title`, `Author`, `book_rating`, `image`, `discription`, `price`, `publisher`)
	VALUES ('$isbn','$title','$author','$rating','$name','$discription','$price','$publisher')";
...
echo $query="UPDATE `books` SET `isbn`='$uisbn',`Title`='$utitle',`Author`='$uauthor',
...
{
$cartid=$_REQUEST['id'];
$n=iud("DELETE FROM `cart` WHERE cartid='$cartid'");
if($n==1)
{
